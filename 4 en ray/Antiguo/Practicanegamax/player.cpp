  ...
  cout << "Llamada" << endl;
   ...
   if (profundidad==0||ganador==0){
    ...
    cout << "Entra";
      ...
      Environment hijo=tablero.GenerateNextMove(act);
         while (act<7){
         double valor= PodaAlfaBeta(hijo,jugador,profundidad-1,LimProfundidad,alpha,beta,false,accion);
         if (valor>=beta)
            return valor;
        if (valor>alpha)
            alpha= valor;
...
int acc;
         ...
         cout << " -> " << actual_.ActionStr( static_cast< Environment::ActionType > (acc)  ) << endl;
