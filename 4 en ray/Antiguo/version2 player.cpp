...
double PodaAlfaBeta(const Environment&tablero, int jugador, int profundidad,int LimProfundidad,double &alpha, double &beta,int &accion,bool jug){
   int ganador=tablero.RevisarTablero();
   ...
   if (profundidad==0||ganador!=0){
    ...
    cout << "Entra";
      ...
      Environment hijo=tablero.GenerateNextMove(act);
        while (act<7){
           if (jug){
                alpha=max(alpha,PodaAlfaBeta(hijo,jugador,profundidad-1,LimProfundidad,alpha,beta,accion,false));
                  if (beta<=alpha)
                    return alpha;
            }
            else{
                beta=min(beta,PodaAlfaBeta(hijo,jugador,profundidad-1,LimProfundidad,alpha,beta,accion,true));
                  if (beta<=alpha)
                  return beta;
...
int act;
            ...
            double jugada = PodaAlfaBeta(actual_,jugador_,PROFUNDIDAD_ALFABETA,PROFUNDIDAD_ALFABETA,alpha,beta,act,true);

         cout << " -> " << actual_.ActionStr( static_cast< Environment::ActionType > (jugada)  ) << endl;
        accion = static_cast< Environment::ActionType > (jugada);
